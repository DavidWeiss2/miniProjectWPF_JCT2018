...
namespace BL
{
    ...
    {
        #region Fields
        private readonly Dal dal_Imp = new Dal();
        #endregion
            ...
            string dateUntil, string isMets, string weekSched)
        {
            ...
            if (errorBuffer == "") //Success befor trying to add
            {
                try
                {
                    ...
                    (bool[] workingDays, int[][] workingHours) = weekSched.ParseWeekSched();
                    this.dal_Imp.Contract.Add(new BE.Contract(this.dal_Imp.Contract.MaxID + 1, nanny, mother, child, perHourBool.ToBool(), dateFrom.FromYYYYMMDD(), dateUntil.FromYYYYMMDD(), workingDays, workingHours, isMets.ToBool(), isSigned.ToBool()));
                }
                catch (Exception e)
                {
                    Append(ref errorBuffer, e.Message);
